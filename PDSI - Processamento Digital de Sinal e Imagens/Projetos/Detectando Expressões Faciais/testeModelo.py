    ...
    if facesPoints:
        for id, detection in enumerate(facesPoints):
            ...
            if probabilityValue >0.30:
                cv2.rectangle(imgOrignal, (x, y), (x + w, y + h), (0,255,0), 18)

                cv2.putText(imgOrignal, str(getCalssName(indexVal)), (x, y-50), cv2.FONT_HERSHEY_SIMPLEX, 4,
                            (0,255,0), 10, cv2.LINE_AA)

                cv2.putText(imgOrignal, str(round(probabilityValue * 100, 2)) + "%", (x, y-20), cv2.FONT_HERSHEY_SIMPLEX, 0.75,(0,255,0), 4,
                            cv2.LINE_AA)

    cv2.imshow("Resultado", cv2.resize(imgOrignal,(0,0),None,0.40,0.40))
